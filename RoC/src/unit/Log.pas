...
uses
  ...
  Main, StdCtrls, ComCtrls, ImgList, Grids, Types,IniFiles,
  ExtCtrls, Menus, LResources, IniPropStorage, TAGraph, LogCompat, TASeries;
...
type
  ...
  private
    ...
    procedure FillBallCenterTreeView(root: TTreeNode; tree:TTreeView);
...
begin
  ...
  Fmain.InsertAuxForms(FLog,'Log');
end;

procedure TFLog.FormClose(Sender: TObject);
begin
  SaveTree(TreeView);
  Fmain.AuxFormClosed('Log');
end;
...
begin
   with tree.Items,LogBuffer[0].RobotState.RobotState,LogBuffer[0].RobotState.Vmotor do begin
    ...
    node.Data:=OffsetTypeToPointer(sv,logDouble);
    node:=AddChild(root,'vn');
    node.Data:=OffsetTypeToPointer(svn,logDouble);
    node:=AddChild(root,'w');
    node.Data:=OffsetTypeToPointer(sw,logDouble);
...
var node: TTreeNode;
begin
  with tree.Items,LogBuffer[1].BallState do begin
    node:=AddChild(root,'x');
    node.Data:=OffsetTypeToPointer(x,logDouble);
    node:=AddChild(root,'y');
    node.Data:=OffsetTypeToPointer(y,logDouble);
...
procedure TFLog.FillBallCenterTreeView(root: TTreeNode; tree: TTreeView);
var node: TTreeNode;
begin
  with tree.Items,LogBuffer[2].centerBall do begin
    node:=AddChild(root,'rho');
    node.Data:=OffsetTypeToPointer(ro,logDouble);
    node:=AddChild(root,'theta');
    node.Data:=OffsetTypeToPointer(ro,logDouble);
    node:=AddChild(root,'theta');
    node.Data:=OffsetTypeToPointer(teta,logDouble);
    node:=AddChild(root,'quality');
    node.Data:=OffsetTypeToPointer(quality,logInt);
  end;
end;
...
begin
  ...
  with tree.Items do begin
    ...
    node:=Add(nil,'BallCenter');
    node.Data:=nil;
    FillBallCenterTreeView(node,tree);
...
begin
  ...
  LogBuffer[LogBufferIn].centerBall:=ObsBallCenter;
    ...
    for i:=0 to Count-1 do begin
      if tree.Items[i].ImageIndex=1 then begin
        ...
        for j:=0 to LogBufferCount-1 do begin
          ...
          Tserie(Chart.Series[cnt-1]).AddXY(j,GetLogValue(tree,i,idx),'',Tserie(Chart.Series[cnt-1]).SeriesColor);
        end;
      end;
    end;
  end;
