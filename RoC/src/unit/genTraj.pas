...
uses
  ...
  //Save circular trajectory definitions
  TTrajectoryCircle = record
    x_center, y_center, radius, length: double;
  end;
  ...
  procedure generateTrajectoryCircle(var traj: TTrajectory; x_center, y_center, radius, length : double);
  procedure generateTrajectory90DegCorner(var traj: TTrajectory; x_start, y_start, L1, L2 : double; varTeta : boolean);
  procedure generateTrajectory8(var traj: TTrajectory; width, height,x_center,y_center : double);
  ...
  //Variables
  var
     TrajectoryCircle : TTrajectoryCircle;
...
implementation
  ...
  // generateTrajectory8
  //
  // Generate a trajectory in 8 shape
  //-------------------------------------------------------------
   procedure generateTrajectory8(var traj: TTrajectory; width, height,x_center,y_center : double);
    var
       i : integer;
       step, teta_inc : double;
  begin

     traj.count:=MaxTrajectoryCount;
     traj.static := true;
     traj.index:=0;
     step := 2*pi/(traj.count-1);
     teta_inc := 0;
     ...
     for i := 0 to (MaxTrajectoryCount-1) do begin
         with traj.pts[i] do begin
              x := x_center+width*cos(teta_inc)/(1+sin(teta_inc)*sin(teta_inc));
              y := y_center+1.35*height*width*sin(teta_inc)*cos(teta_inc)/(1+sin(teta_inc)*sin(teta_inc));
              teta := 0;
              teta_power:= 1;
              teta_inc += step;
         end;
 ...
 //*************************************************************
  // generateTrajectoryL
  //
  // Generates a L trajectory
  //-------------------------------------------------------------
  procedure generateTrajectory90DegCorner(var traj: TTrajectory; x_start, y_start, L1, L2 : double; varTeta : boolean);
  var
     i,cornerPoint,cornerPoint2 : integer;
     tetaRef : array[0 .. 2] of double;
     tetaInc : array[0 .. 1] of double;
  begin
  ...
  if varTeta then begin
     ...
     if L1 > 0 then
        tetaInc[0] := (tetaRef[1]-tetaRef[0])/(round(abs(L1)/trajStep));
     if L2 > 0 then
        tetaInc[1] := (tetaRef[2]-tetaRef[1])/(round(abs(L2)/trajStep));
     ...
     cornerPoint := round(abs(L1)/trajStep);
     cornerPoint2 := cornerPoint + round(abs(L2)/trajStep);

     traj.pts[0].x := x_start;
     traj.pts[0].y := y_start;
     traj.pts[0].teta := tetaRef[0];

     for i:= 1 to cornerPoint do begin
         if (i <= MaxTrajectoryCount-1) then begin
          with traj.pts[i] do begin
              x := x_start;
              y := traj.pts[i-1].y + Sign(L1)*trajStep;
              teta := traj.pts[i-1].teta + tetaInc[0];
              teta_power:= 1;
          end;
         end else begin
             calcDistance(traj);
     ...
     for i:= cornerPoint+1 to cornerPoint2 do begin
         if (i <= MaxTrajectoryCount-1) then begin
            with traj.pts[i] do begin
              x := traj.pts[i-1].x + Sign(L2)*trajStep;
              y := traj.pts[i-1].y;
              teta := traj.pts[i-1].teta + tetaInc[1];
              teta_power:= 1;
            end;
         end else begin
             calcDistance(traj);
