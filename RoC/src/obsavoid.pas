...
uses Main,DecConsts,SysUtils,Field;
...
begin
  ...
  if Params[paramDrawObstacleAvoid] then begin
    ...
    FMain.ImageMap.Canvas.Pen.Color:=clLime;
    //WorldToField(st_x,st_y,pixx1,pixy1);
    WorldToMap(st_x,st_y,pixx1,pixy1);
  ...
  while not done do begin
    ...
    if Params[paramDrawObstacleAvoid] then begin
      ...
      WorldToMap(px,py,pixx1,pixy1);
      FMain.ImageMap.Canvas.LineTo(pixx1,pixy1);
    end;
...
begin
  ...
  if Params[paramDrawObstacleAvoid] then begin
    FMain.ImageMap.Canvas.Pen.Style:=psSolid;
    FMain.ImageMap.Canvas.Brush.Style:=bsClear;
    FMain.ImageMap.Canvas.Pen.Color:=clGray;
    for i:=0 to num_obs-1 do begin
      //WorldToField(obs[i].x-obs[i].r,obs[i].y-obs[i].r,pixx1,pixy1);
      //WorldToField(obs[i].x+obs[i].r,obs[i].y+obs[i].r,pixx2,pixy2);
      WorldToMap(obs[i].x-obs[i].r,obs[i].y-obs[i].r,pixx1,pixy1);
      WorldToMap(obs[i].x+obs[i].r,obs[i].y+obs[i].r,pixx2,pixy2);
      FMain.ImageMap.Canvas.Ellipse(pixx1,pixy1,pixx2,pixy2);
    end;
  end;
...
function RobotBestPath(num: integer; tg_x, tg_y:double; var traj: TTrajectory; avoid: TAvoidSet): boolean;
var obs: array[0..MaxRobots+MaxOponents+7+MaxObstacles] of TRoundObstacle;  //Acke provisório o mais 70
    i,nobs: integer;
    dball,r1: double;
...
begin
  ...
  if (avoidBall in avoid) then begin
  ...
  if avoidOurArea in avoid then begin
    obs[nobs].x := OurGoalX;
    obs[nobs].y := 0;
    obs[nobs].r :=FieldDims.KeeperAreaDepth + RobotSpace / 2 + 0.03;
    inc(nobs);
    obs[nobs].x := OurGoalX;
