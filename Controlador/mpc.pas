...
uses
  ...
  obsavoid, Utils, model, Math, Roles, DecConsts;
...
var
  ...
  //for obstacles???
  Obst: array[0..MaxRobots+MaxOponents+7+MaxObstacles] of TRObstacle;
  final: integer = MaxRobots+MaxOponents+7+MaxObstacles;
...
begin
  ...
  //Add link to menu in main form
  {$ifdef MDEC}
  Fmain.InsertAuxForms(FormMPC,'Formation Control');
  {$endif}
end;

procedure TFormMPC.FormClose(Sender: TObject; var CloseAction: TCloseAction);
begin
  {$ifdef MDEC}
  Fmain.AuxFormClosed('Formation Control');
  {$endif}
end;
...
begin
     ...
     if CheckBoxPropSat.Checked then
...
//          values.
//----------------------------------------------------------------------------
function TFormMPC.predSimulator(var Robot : TSimRobot; var U : TDMatrix; refTraj: TTrajectory) : double;
var
...
begin
     ...
     for i:= SimParameters.N1 to SimParameters.N2 do begin
         ...
         for j:= 0 to 3 do begin
             ...
             if CheckBoxSimDynamics.Checked then begin
         ...
         //calculate errors
         sum_x := sum_x + (refTraj.pts[i].x-Robot.RobotState.x)*(refTraj.pts[i].x-Robot.RobotState.x);
         sum_y := sum_y + (refTraj.pts[i].y-Robot.RobotState.y)*(refTraj.pts[i].y-Robot.RobotState.y);
         sum_teta := sum_teta + DiffAngle(refTraj.pts[i].teta,Robot.RobotState.teta)*DiffAngle(refTraj.pts[i].teta,Robot.RobotState.teta);
     end;

     deltaU:= power((Robot.RobotState.v-U.getv(0,0)),2) + power((Robot.RobotState.vn-U.getv(1,0)),2) + power((Robot.RobotState.w-U.getv(2,0)),2);

     //Output cost
     result := SimParameters.lambda[0]*(sum_x + sum_y) + SimParameters.lambda[1]*sum_teta + SimParameters.lambda[2]*(deltaU*deltaU);
end;
