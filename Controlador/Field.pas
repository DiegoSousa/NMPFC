...
uses
  ...
  StdCtrls, LResources, IniPropStorage, DecConsts;
...

{
  ...
  TFieldVLines=record
    Lines: array[0..MaxFieldLines-1] of TFieldVLine;
    count: integer;
  end;
  ...
  TFieldHLines=record
    Lines: array[0..MaxFieldLines-1] of TFieldHLine;
    count: integer;
  end;
}
...
//  FieldVlines: TFieldVLines;
//  FieldHlines: TFieldHLines;
...
{
procedure addVline(nx,ny1,ny2,nLWidth: double);
begin
  if FieldVlines.count<MaxFieldLines-1 then begin
    with FieldVlines.Lines[FieldVlines.count] do begin
      x:=nx;
      y1:=ny1;
      y2:=ny2;
    end;
    inc(FieldVlines.count);
  end;
end;
}
