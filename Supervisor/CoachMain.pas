...
begin
  if CBKeeperActive.Checked then begin
     ...
     //RobotStatus[num].default_role:=KeeperWay;
  end else begin
...
begin
  ...
  if BestBallIdx>=0 then begin
    with BallState do begin
      x := BallStates[BestBallIdx].x;
      y := BallStates[BestBallIdx].y;
      quality:=BallStates[BestBallIdx].quality;
      vx:= BallStates[BestBallIdx].vx;
      vy:= BallStates[BestBallIdx].vy;
...
begin
  ...
  with BallStates[PlayerInfo.num] do begin
    ...
    quality := PlayerInfo.BallState.conf;
  end;
