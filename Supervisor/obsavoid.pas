...
uses Coach,DecConsts,SysUtils,Field;

const
  MaxTrajectoryCount = 128;

type
  TAvoid=(avoidBall,avoidOponent,avoidRobot,avoidOurArea,avoidObstacles);
...
const
  ...
  AStarSlack = 0.1;
  //manuel
  TotalFieldWidth = (FieldWidth + FieldOutsideSpace * 2 - (RSpace + AStarSlack));
  TotalFieldLength = (FieldLength + FieldOutsideSpace * 2 - (RSpace + AStarSlack));
...
uses Graphics, Robots, Roles, Tactic, Utils, Param, Dialogs;
...
begin
  ...
  // TODO : Draw obs
  if Params[paramDrawObstacleAvoid] then begin
    FCoach.ImageMap.Canvas.Pen.Style:=psSolid;
    FCoach.ImageMap.Canvas.Brush.Style:=bsClear;
    FCoach.ImageMap.Canvas.Pen.Color:=clLime;
    //WorldToField(st_x,st_y,pixx1,pixy1);
    FCoach.WorldToMap(st_x,st_y,pixx1,pixy1);
    FCoach.ImageMap.Canvas.MoveTo(pixx1,pixy1);
  end;
  ...
  while not done do begin
    ...
    // TODO : Draw obs
    if Params[paramDrawObstacleAvoid] then begin
      //WorldToField(px,py,pixx1,pixy1);
      FCoach.WorldToMap(px,py,pixx1,pixy1);
      FCoach.ImageMap.Canvas.LineTo(pixx1,pixy1);
    end;
...
begin
  ...
  if Params[paramDrawObstacleAvoid] then begin
    FCoach.ImageMap.Canvas.Pen.Style:=psSolid;
    FCoach.ImageMap.Canvas.Brush.Style:=bsClear;
    FCoach.ImageMap.Canvas.Pen.Color:=clGray;
    for i:=0 to num_obs-1 do begin
      FCoach.WorldToMap(obs[i].x-obs[i].r,obs[i].y-obs[i].r,pixx1,pixy1);
      FCoach.WorldToMap(obs[i].x+obs[i].r,obs[i].y+obs[i].r,pixx2,pixy2);
      FCoach.ImageMap.Canvas.Ellipse(pixx1,pixy1,pixx2,pixy2);
    end;
  end;
...
function RobotBestPath(num: integer; tg_x, tg_y:double; var traj: TTrajectory; avoid: TAvoidSet): boolean;
var obs: array[0..MaxRobots+MaxOponents+7] of TRoundObstacle;
    i,nobs: integer;
    dball,r1: double;
...
begin
  ...
  // TODO : oponent
  if avoidOponent in avoid then begin
    for i := 0 to MaxOponents - 1 do begin
      if OponentState[i].valid then begin
        dball := Dist(OponentState[i].x - BallState.x, OponentState[i].y - BallState.y);
        r1:=RobotSpace;
        obs[nobs].x := OponentState[i].x;
        obs[nobs].y := OponentState[i].y;
        obs[nobs].r := r1;
        inc(nobs);
      end;
    end;
  ...
  if (avoidBall in avoid) {or (teamRole in StoppedBarrierRoleSet)} then begin
    obs[nobs].x:=BallState.x;
    obs[nobs].y:=BallState.y;
    obs[nobs].r:=BallSpace;
    inc(nobs);
  end;

  if avoidOurArea in avoid then begin
    obs[nobs].x := OurGoalX;
    obs[nobs].y := 0;
    obs[nobs].r :=FieldDims.AreaDepth + RobotSpace / 2 + 0.03;
    inc(nobs);
    obs[nobs].x := OurGoalX;
    obs[nobs].y := 0+FieldDims.AreaWidth/2;
    obs[nobs].r :=FieldDims.AreaDepth + RobotSpace / 2 + 0.03;
    inc(nobs);
    obs[nobs].x := OurGoalX;
    obs[nobs].y := 0-FieldDims.AreaWidth/2;
  ...
  //manuel
   //for i:=round(-GoalWidth) to round(GoalWidth) do begin
  ...
  //for i:=round(TheirGoalX) to round(TheirGoalX+GoalDepth) do begin
  ...
  //for i:=round(OurGoalX) to round(OurGoalX+GoalDepth) do begin
    obs[nobs].x := OurGoalX;
    obs[nobs].y := -FieldDims.GoalWidth+1;
    obs[nobs].r := 0.6;
    inc(nobs);
  ...
  if Params[paramDrawObstacleAvoid] then begin
{  if (tg_x<>px) or (tg_y<>py) then begin
    avoided[num]:=true;
    avoided_x[num]:=px;
    avoided_y[num]:=py;
  end;}
{    FMain.ImageMap.Canvas.Pen.Style:=psSolid;
    FMain.ImageMap.Canvas.Brush.Style:=bsClear;
    FMain.ImageMap.Canvas.Pen.Color:=clWhite;
    WorldToMap(RobotState[num].x,RobotState[num].y,pixx1,pixy1);
    WorldToMap(tg_x,tg_y,pixx2,pixy2);
    FMain.ImageMap.Canvas.MoveTo(pixx1,pixy1);
    FMain.ImageMap.Canvas.LineTo(pixx2,pixy2);}
  end;
