...
uses
  SysUtils, Classes, Graphics, Controls, Forms, Dialogs, Coach, StdCtrls,
...
type
  ...
  LogRecord=record
    ...
    RobotState: array[0..4] of TRobotState;
    centerBall:TCenter;
  end;
...
var ini: TIniFile;
    i: integer;
begin
  ini:=TIniFile.Create(extractfilepath(application.ExeName)+FCoach.dataDir+'/tree.ini');
...
begin
  ...
  with tree.Items do begin
    ...
    node:=Add(nil,'Robot 5');
    node.Data:=nil;
    FillRobotStateTreeView(4,node,tree);
...
begin
  ...
  LogBuffer[LogBufferIn].RobotState[4]:=RobotState[4];
