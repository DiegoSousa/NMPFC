...
begin
  if CBMShow.Checked then begin
    ...
    LoadMap(ExtractFilePath(Application.ExeName)+FCoach.DataDir+'/maps/'+EditMapLoadName.Text+'.dst');
...
begin
  FCoach.AuxFormClosed('LocMap');
end;

procedure TFLocMap.FormCreate(Sender: TObject);
begin

  FCoach.InsertAuxForms(FLocMap,'LocMap');
  CBMShowChange(Self);
end;
...
var
  r:localization.TPos;
begin
  {if FCoach.CBLocalization.Checked then begin
